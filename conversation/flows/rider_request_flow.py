...
def view_ride_requests(session):
    ...
    requests = (
        RideBookingResponse.objects.filter(rider=session.user, response="PENDING")
        .select_related("booking", "route")
        .order_by("-created_at")
    )
...
def accept_ride_request(session, response_id):
    ...
    ride_response.updated_price = ride_response.booking.estimated_price
    ...
    return send_text(session.phone_number, "Ride request accepted ✅")


def reject_ride_request(session, response_id):

    ride_response = RideBookingResponse.objects.filter(
        id=response_id, rider=session.user, response="PENDING"
    ).first()
    ...
    ride_response.response = "REJECTED"

    ride_response.save()

    return send_text(session.phone_number, "Ride request rejected.")


def offer_new_price(session, response_id, price):
...
# =========================================
def handle_driver_request_action(session, message):
    ...
    accept_match = re.match(r"^accept\s+(\d+)$", normalized_message)

    if accept_match:

        response_id = int(accept_match.group(1))
    ...
    reject_match = re.match(r"^reject\s+(\d+)$", normalized_message)

    if reject_match:

        response_id = int(reject_match.group(1))
    ...
    offer_match = re.match(r"^offer\s+(\d+)\s+(\d+)$", normalized_message)

    if offer_match:

        response_id = int(offer_match.group(1))

        updated_price = int(offer_match.group(2))

        return offer_new_price(session, response_id, updated_price)
...
def view_ride_offers(session):
    ...
    if not booking:
        ...
        return send_text(
            session.phone_number, ("You do not have any " "active ride requests.")
        )

    responses = (
        RideBookingResponse.objects.filter(booking=booking, response="ACCEPTED")
    ...
    if not responses.exists():
        ...
        return send_text(
            session.phone_number, ("No drivers have responded " "to your ride yet.")
        )

    message = "🚘 Driver Offers\n\n"
    ...
    session.context = {
        "active_flow": (RIDE_OFFER_FLOW),
        "step": (SELECTING_RIDER),
...
# =========================================
def handle_rider_selection(session, message):
    ...
    booking_id = session.context["data"].get("booking_id")

    booking = RideBooking.objects.filter(id=booking_id, passenger=session.user).first()
    ...
    if not responses:
        ...
        return send_text(session.phone_number, ("No accepted driver offers found."))

    # =====================================
    # VALIDATE SELECTION
    # =====================================

    if selected_index < 1 or selected_index > len(responses):
    ...
    selected_response = responses[selected_index - 1]

    # =====================================
    # FINALIZE BOOKING
    # =====================================

    booking.selected_rider = selected_response.rider

    booking.final_price = selected_response.updated_price or booking.estimated_price
    ...
    RideBookingResponse.objects.filter(booking=booking).exclude(
