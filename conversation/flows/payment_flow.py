...
from rideshare.utils.paystack_service import initialize_paystack_payment


def handle_payment_flow(session, message):

    normalized_message = message.strip().lower()

    booking_id = session.context["data"].get("booking_id")

    booking = RideBooking.objects.filter(id=booking_id).first()
    ...
    if normalized_message == "pay now":
        ...
        payment = initialize_paystack_payment(booking)
    ...
    return send_text(
        session.phone_number, ("Reply with:\n\n" "• pay now\n" "• cancel ride")
    )
