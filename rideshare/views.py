...
def paystack_webhook(request):
    ...
    computed_signature = hmac.new(
        os.getenv("PAYSTACK_SECRET_KEY").encode(), request.body, hashlib.sha512
    ).hexdigest()
    ...
    payment = (
        Payment.objects.select_related("booking", "passenger", "rider")
    ...
    passenger_session = ConversationSession.objects.filter(
        user=booking.passenger
    ).first()

    if passenger_session:

        reset_session(passenger_session)

    # =====================================
    # NOTIFY PASSENGER
    # =====================================

    send_passenger_payment_success_message(booking)

    # =====================================
    # RESET PASSENGER SESSION
    # =====================================

    rider_session = ConversationSession.objects.filter(
        user=booking.selected_rider
    ).first()

    if rider_session:

        reset_session(rider_session)

    # =====================================
    # NOTIFY RIDER
    # =====================================

    send_rider_payment_success_message(booking)
